const { enrichItemWithRules } = require("./packingRulesEngine");
...
const getLayoutSection = (item, index) => {
  ...
  if (zone === "bottom or edges") return index % 2 === 0 ? "leftSide" : "rightSide";

  return index % 2 === 0 ? "leftSide" : "rightSide";
};

const getSmartAdvice = (items, totals) => {
  const advice = [];
  ...
  if (!totals.overallFits) {
    advice.push("Your selected items may not fit well in your trip bags.");
  }

  if (!totals.volumeFits) {
    advice.push("Your packing volume is over the available bag limit. Reduce bulky items first.");
  }

  if (!totals.weightFits) {
    advice.push("Your luggage may exceed the allowed weight limit. Remove heavier items first.");
  }

  if (totals.usedCapacityPercent >= 90 && totals.usedCapacityPercent <= 100) {
    advice.push("Your bags are almost full. Leave a little extra space for easier closing.");
  }

  if (totals.usedCapacityPercent > 100) {
    advice.push("Your bags are overfilled. Consider a larger setup or fewer items.");
  }

  const weightUsagePercent =
    totals.totalAllowedWeightG > 0
      ? (totals.totalWeightG / totals.totalAllowedWeightG) * 100
      : 0;

  if (weightUsagePercent >= 85 && weightUsagePercent <= 100) {
    advice.push("Your luggage is getting close to the total weight limit.");
  }
...
const buildAdjustmentSuggestions = (items, totals) => {
  ...
  if (mainConstraint === "volume" || mainConstraint === "both") {
    ...
    if (bulky[0]) {
      adjustments.push(`Remove or reduce ${bulky[0].name} first to lower suitcase volume.`);
    }

    if (bulky[1]) {
      adjustments.push(`Consider reducing ${bulky[1].name} if you still need more space.`);
    }
    ...
    if (foldableLarge) {
      adjustments.push(`Reduce the quantity of ${foldableLarge.name} to free up more packing space.`);
    }

    warnings.push("Your main packing issue is volume.");
  }
  ...
  if (mainConstraint === "weight" || mainConstraint === "both") {
    ...
    if (heavy[0]) {
      adjustments.push(`Remove or reduce ${heavy[0].name} first to lower total weight.`);
    }

    if (heavy[1]) {
      adjustments.push(`Check whether ${heavy[1].name} is necessary for this trip.`);
    }
    ...
    if (heavyCustom) {
      adjustments.push(`Review the custom item ${heavyCustom.name}, as it adds significant weight.`);
    }

    warnings.push("Your main packing issue is weight.");
  }

  if (totals.usedCapacityPercent >= 85 && totals.usedCapacityPercent <= 100) {
    optimizationTips.push("Your bags are close to full capacity. Keep a little free space for easier closing.");
  }
  ...
  if (weightUsagePercent >= 85) {
    optimizationTips.push("Your packed weight is close to the allowed limit. Avoid adding more heavy items.");
  }
  ...
  if (shoes && shoes.quantity >= 1) {
    optimizationTips.push("Shoes take a large amount of space. Pack only the pairs you really need.");
  }
...
const chooseTargetBagRole = (item) => {
  ...
  if (category === "shoes" || category === "outerwear" || behavior === "rigid") {
    return "main";
  }
...
const buildBagDistribution = (items, suitcases) => {
  ...
  for (const item of items) {
    ...
    targetBag.items.push({
      ...
      assignedBagId: item.assignedBagId || null,
      assignmentMode: item.assignedBagId ? "manual" : "auto",
      priority: item.priority,
      removePriority: item.removePriority,
      preferredBagRole: item.preferredBagRole,
    });
...
const isBagOverloadedOrTight = (bag) => {
  return !bag.volumeFits || !bag.weightFits || bag.usedCapacityPercent >= 85;
};
...
const buildBagRebalancingSuggestions = (bagDistribution) => {
  ...
  const targetBags = bagDistribution.filter(
    (bag) => bag.volumeFits && bag.weightFits && bag.usedCapacityPercent < 75
  );
...
const calculateTripResult = ({ suitcases, tripItems, sizeMultipliers, tripMeta = {} }) => {
  ...
  const detailedItems = tripItems.map((item) => {
    ...
    const itemVolume = Number(item.base_volume_cm3 || 0) * sizeMultiplier * quantity;
    const itemWeight = Number(item.base_weight_g || 0) * sizeMultiplier * quantity;
    ...
    return enrichItemWithRules({
      ...
      finalVolumeCm3: itemVolume,
      finalWeightG: itemWeight,
  ...
  const totals = {
    ...
    totalAllowedWeightG: totalAvailableWeightG,
  };
  ...
  const advice = getSmartAdvice(detailedItems, totals);
  const smartAdjustments = buildAdjustmentSuggestions(detailedItems, totals);
